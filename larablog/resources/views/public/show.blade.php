<x-guest-layout>
    <div class="w-2/5 mx-auto bg-white dark:bg-gray-800">
        ...
        <div class="text-gray-500 text-sm flex items-center justify-between">
            <div>
                Publié le {{ $article->created_at->format('d/m/Y') }} par
                <a href="{{ route('public.index', $article->user->id) }}">{{ $article->user->name }}</a>
            </div>
            ...
            <div>
                @auth
                    <a href="{{ route('articles.like', $article->id) }}"
                        class="block w-10 bg-white border border-red-500 hover:bg-red-100 text-white font-bold p-2 rounded">
                        ...
                        <span class="block text-red-500 text-center">{{ $article->likes }}</span>
                    </a>
                @endauth
            </div>
        </div>
